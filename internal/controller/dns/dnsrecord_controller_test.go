...
func TestReconcileRendersZoneConfigMap(t *testing.T) {
	...
	if err := client.Get(context.Background(), types.NamespacedName{
		Name:      dnsdomain.ZoneConfigMapName,
		Namespace: "dns-operator-system",
	}, &configMap); err != nil {
		t.Fatalf("get configmap: %v", err)
	}

	content := configMap.Data[dnsdomain.ZoneConfigMapKey]
	if content == "" {
	...
	if updatedRecord.Status.ZoneConfigMapName != dnsdomain.ZoneConfigMapName {
		t.Fatalf("expected zone configmap name %s, got %s", dnsdomain.ZoneConfigMapName, updatedRecord.Status.ZoneConfigMapName)
	}
